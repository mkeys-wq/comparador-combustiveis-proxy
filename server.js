...
app.get('/api/debug/find-options', async (req, res) => {
  try {
    const r = await fetch('https://precoscombustiveis.dgeg.gov.pt/postos/', {
    ...
    const selectMatch = html.match(/<select[^>]*id="cboTipoCombustivel"[\s\S]*?<\/select>/);

    res.json({
      totalOptionsFound: options.length,
      allOptions: options,
      tipoCombustivelBlock: selectMatch ? selectMatch[0] : '(não encontrado)',
    });
  } catch (err) {
    res.status(502).json({ error: String(err.message || err) });
  }
